<template>
  <div>
    ...
    <div class="wg-form-registration__card-imgs" v-if="lincksPhoto!=undefined">
      <wg-form-create-shop-photo-img
        v-for="(val, key) in lincksPhoto"
        :key="key"
        :keyPhoto="key"
        :linck="val"
        :ad="dShop"
        :checkMain="dShop.main_photo==shopsPhotoHost+'/'+val?true:false"
...
import WgFormCreateShopPhotoImg from "./card-photo-img.vue";
export default {
  components: {
    WgFormCreateShopPhotoImg
  },
  data() {
    return {
      dShop: this.shop,
      ...
      imgSpinner: false,
      shopsPhotoHost: this.$hosts.shopsPhoto
    };
  },
  computed: {
